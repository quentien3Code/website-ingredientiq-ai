...
class IngredientFullDataView(APIView):
    ...
    def _search_youtube_videos(self, query):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(videos) == 0:
                    print("⚠️ No reputable videos found, including general health videos")
    ...
    def _fallback_badge_analysis(self, ingredient, user_profile, regulatory_data):
        ...
        if user_allergies and any(allergen in ingredient_lower for allergen in user_allergies.split(',')):
            ...
            badge = {'color': 'yellow', 'text': 'Caution', 'icon': '⚠️'}
...
class BarcodeView(APIView):
    ...
    def _implement_enhanced_analysis_flow(self, user, ingredients_list, nutrition_data):
        ...
        overall_assessment = "✅ Safe" if len(flagged_ingredients) == 0 else "⚠️ Caution" if len(flagged_ingredients) < 3 else "❌ No-Go"
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            try:
                ...
                ibs_fodmap_analysis = {
                    ...
                    "analysis": [],
                    "severity_sliders": {"High FODMAP": "🚫 Avoid", "Moderate FODMAP": "⚠️ Limit", "Low FODMAP": "✅ Monitor"},
                ...
                ai_results = {
                    ...
                    "expert_ai_conclusion": {
                        ...
                        "scoring_transparency": "Life-threatening allergens override other factors → No-Go, even if other conditions return Caution",
                        "ui_recommendations": {
                            "overall_badge": "🚫 No-Go" if risk_level == "high" else "⚠️ Caution" if risk_level == "moderate" else "✅ Safe",
...
Do not add extra headings or commentary. Keep the total ≤ 270 words."""
    ...
    def _implement_enhanced_analysis_flow(self, user, ingredients_list, nutrition_data):
        ...
        unified_analysis = {
            "overall_assessment": "🚫 No-Go" if any(score['weighted_score'] >= 75 for score in weighted_scoring['ingredient_scores']) else "⚠️ Caution" if any(score['weighted_score'] >= 50 for score in weighted_scoring['ingredient_scores']) else "✅ Safe",
            "condition_specific_breakdown": condition_specific_mapping,
            "weighted_scoring_breakdown": weighted_scoring,
            "scoring_transparency": "Life-threatening allergens override other factors → No-Go, even if other conditions return Caution"
        }
    ...
    def clean_ingredient_text(self, ingredient):
        ...
        # Remove bullet points
        ingredient = re.sub(r'^\s*[-â€¢]\s*', '', ingredient)
